...
class RandomCenterCornerCrop(SpatialTransform):
    def __init__(self, size, interpolation=Image.BILINEAR, crop_positions=('c', 'tl', 'tr', 'bl', 'br')):
...
class ToTensor(SpatialTransform):
    ...
    def __call__(self, pic):
        ...
        if pic.mode != "RGB":
            ...
            img      = pic.tobytes()
            img      = torch.ByteStorage.from_buffer(img)
...
class Preprocessing(SpatialTransform):
    def __init__(self, size, degrees=20, brightness=0.5, is_aug=True, center=False):
        ...
        self.f2     = RandomApply(RandomChoice([HorizontalFlip(), RandomRotation(degrees), BrightnessJitter(brightness)]), prob=0.3)
